'use strict';
// Each word retains its original sequence of plain and italic text runs.
function tokenize(runs){const result=[];let parts=[];function flush(){if(parts.length){result.push({text:parts.map(p=>p.text).join(''),parts});parts=[];}}for(const run of runs){for(const piece of run.text.split(/(\s+)/u)){if(!piece)continue;if(/^\s+$/u.test(piece)){flush();if(/[\r\n]/u.test(piece)&&result.length)result.at(-1).paragraphEnd=true;continue;}const italic=!!run.italic,last=parts.at(-1);if(last&&last.italic===italic)last.text+=piece;else parts.push({text:piece,italic});}}flush();return result;}
function parseXml(text){const doc=new DOMParser().parseFromString(text,'application/xml');if(doc.getElementsByTagName('parsererror').length)throw Error('This document contains unreadable XML. Try saving a fresh copy.');return doc;}
const localElements=(el,name)=>[...el.getElementsByTagNameNS('*',name)];
class BookZip{
 constructor(buffer){this.bytes=new Uint8Array(buffer);this.view=new DataView(buffer);this.entries=new Map();this.total=0;let end=-1;for(let i=buffer.byteLength-22;i>=Math.max(0,buffer.byteLength-65557);i--){if(this.view.getUint32(i,true)===0x06054b50){end=i;break;}}if(end<0)throw Error('This file is not a readable DOCX or EPUB archive.');let pos=this.view.getUint32(end+16,true);const count=this.view.getUint16(end+10,true);for(let n=0;n<count;n++){if(this.view.getUint32(pos,true)!==0x02014b50)throw Error('The document archive is damaged.');const nameLength=this.view.getUint16(pos+28,true),name=new TextDecoder().decode(this.bytes.subarray(pos+46,pos+46+nameLength));this.entries.set(name,{flags:this.view.getUint16(pos+8,true),method:this.view.getUint16(pos+10,true),length:this.view.getUint32(pos+20,true),size:this.view.getUint32(pos+24,true),offset:this.view.getUint32(pos+42,true)});pos+=46+nameLength+this.view.getUint16(pos+30,true)+this.view.getUint16(pos+32,true);}}
 async text(name){const entry=this.entries.get(name);if(!entry)throw Error(`A required part of this document is missing: ${name}`);if(entry.flags&1)throw Error('Password-protected or DRM-protected books cannot be opened. Use an unprotected copy.');if(entry.size>30*1024*1024)throw Error('A section is too large. Try a smaller document.');const o=entry.offset,start=o+30+this.view.getUint16(o+26,true)+this.view.getUint16(o+28,true);if(start+entry.length>this.bytes.length)throw Error('The document archive is incomplete.');const data=this.bytes.slice(start,start+entry.length);let stream=new Blob([data]).stream();if(entry.method===8){try{stream=stream.pipeThrough(new DecompressionStream('deflate-raw'));}catch{throw Error('Update your browser to open this file, or paste the text instead.');}}else if(entry.method!==0)throw Error('This archive uses an unsupported compression format.');const reader=stream.getReader(),chunks=[];let size=0;while(true){const {value,done}=await reader.read();if(done)break;size+=value.length;this.total+=value.length;if(size>30*1024*1024||this.total>80*1024*1024){await reader.cancel();throw Error('This book is too large. Try a smaller document.');}chunks.push(value);}return new Blob(chunks).text();}
}
const W='http://schemas.openxmlformats.org/wordprocessingml/2006/main';
function wAttr(el,name){return el?.getAttributeNS(W,name)||el?.getAttribute('w:'+name)||'';}
function child(el,name){return el?[...el.children].find(c=>c.localName===name):null;}
function italicSetting(properties){const i=child(properties,'i');if(!i)return undefined;return !['0','false','off'].includes(wAttr(i,'val'));}
async function readDocx(buffer){const zip=new BookZip(buffer),doc=parseXml(await zip.text('word/document.xml'));const styles=zip.entries.has('word/styles.xml')?parseXml(await zip.text('word/styles.xml')):null;const styleMap=new Map(localElements(styles||doc,'style').map(s=>[wAttr(s,'styleId'),s]));const defaultStyle=styles?localElements(styles,'style').find(s=>wAttr(s,'type')==='paragraph'&&wAttr(s,'default')==='1'):null;const defaults=styles?italicSetting(localElements(styles,'rPrDefault')[0]?.firstElementChild):undefined;
 function styleValue(id,value,seen=new Set()){const s=styleMap.get(id);if(!s||seen.has(id))return value;seen.add(id);value=styleValue(wAttr(child(s,'basedOn'),'val'),value,seen);const setting=italicSetting(child(s,'rPr'));return setting===true?!value:value;}
