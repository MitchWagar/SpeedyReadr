...
let currentBookId=null,chapters=[],words=[],index=0,timer=null,playing=false,finished=false,keyboard=false,uploadVersion=0,title="Sample text",paragraphVersion=1;
...
function syncDelete(){const included=$('bookmarks').value===FEATURED_ID;$('deleteBookmark').hidden=included;$('deleteBookmark').disabled=included;$('restartFeatured').hidden=!included;}
...
function beginLocalBook(){const snapshot={title,words,chapters,index,paragraphVersion,speed:Number($('speed').value)};activeBook=queueStorage(async()=>{const digest=await crypto.subtle.digest('SHA-256',new TextEncoder().encode(JSON.stringify(snapshot.words.map(({text,parts})=>({text,parts})))));const id=Array.from(new Uint8Array(digest),b=>b.toString(16).padStart(2,'0')).join('');if(words===snapshot.words)currentBookId=id;await api('/'+id,{method:'PUT',body:JSON.stringify(snapshot)});await refreshBookmarks();$('bookmarkMessage').textContent='Document saved on this device. Your position saves automatically.';return id;});activeBook.catch(storageError);}
...
function render(){syncChapter();$('sampleEnd').hidden=!(currentBookId===FEATURED_ID&&(finished||index===words.length-1)); $('featuredOpening').hidden=!showOpening;$('word').hidden=showOpening;$('pad').classList.toggle('opening',showOpening); $('word').replaceChildren(); for(const part of (words[index]?.parts||[{text:'Ready.',italic:false}])){const span=document.createElement(part.italic?'em':'span');span.textContent=part.text;$('word').append(span);} $('position').textContent=words.length?`${index+1} / ${words.length.toLocaleString()} words`:'0 words';const pct=finished?100:words.length>1?Math.round(index/(words.length-1)*1000)/10:0;$('percent').textContent=pct+'%';$('pad').classList.toggle('active',playing);$('holdHint').textContent=finished?'Finished · rewind or jump to 0% to read again':playing?'Release to pause & rewind 10':'Press and hold here to read';fit();scheduleLocalSave();}
...
function load(input,label,saveLocally=true,chapterList=input?.chapters){const next=typeof input==='string'?tokenize([{text:input}]):input;if(!next.length){$('message').textContent='Add some text before loading the reader.';return false;}viewVersion++;resetInput();activeBook=null;currentBookId=null;showOpening=false;words=next;index=0;finished=false;title=label;paragraphVersion=1;setChapters(chapterList);$('sourceCount').textContent=label;$('message').textContent=`${words.length.toLocaleString()} words loaded. Hold the reading area to begin.`;render();if(saveLocally)beginLocalBook();return true;}

$('clearText').onclick=()=>{viewVersion++;uploadVersion++;$('text').value='';$('file').value='';$('message').textContent='Paste area cleared. Add new text when ready.';$('text').focus({preventScroll:true});};
$('load').onclick=()=>{uploadVersion++;const text=$('text').value;load(text,'Pasted text');};
...
$('restartFeatured').onclick=async()=>{
  if($('bookmarks').value!==FEATURED_ID)return;
  if(!window.confirm('Restart Cold-Keep Reprisal from the beginning? Your saved place in this book will be reset.'))return;
  uploadVersion++;load(FEATURED_BOOK,'Cold-Keep Reprisal — James Lurid',false);
  currentBookId=FEATURED_ID;showOpening=true;
  const snapshot={...featuredSnapshot(),speed:Number($('speed').value),finished:false};
  activeBook=queueStorage(async()=>{await api('/'+FEATURED_ID,{method:'PUT',body:JSON.stringify(snapshot)});return FEATURED_ID;});
  render();
  try{await activeBook;await refreshBookmarks();$('bookmarkMessage').textContent='Cold-Keep Reprisal restarted. Hold the reading box to begin.';}catch(error){storageError(error);}
};

load(FEATURED_BOOK,'Cold-Keep Reprisal — James Lurid',false);
currentBookId=FEATURED_ID;
showOpening=true;$('text').value='';
